...
public class DBConnect {
    ...
    public static Boolean AddNewCustomerReservation (Customer customer) throws TimeIsFullException {
        Connection conn = ConnectDb();
        ...
        else if (checkDate(formattedDate) >= 2) {
            throw new TimeIsFullException("Tento čas je plně obsazen!");
        }
        return false;
    }

    public static int checkDate(String datetime){
        Connection conn = ConnectDb();
        int count = 0;
        try {
            assert conn != null;
            Statement statement = conn.createStatement();
            ResultSet rs = statement.executeQuery("SELECT date FROM customer");
            while (rs.next()) {
                if ((rs.getDate("date").toString() + " " + rs.getTime("date").toString()).equals(datetime)){
                    count++;
                }
            }
    ...
    /*public static int checkDate(Customer customer){
        Connection conn=ConnectDb();
        int count = 0;
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm");
        var formattedDate = sdf.format(customer.getDate());
        ...
        try {
            ...
            ResultSet rs = statement.executeQuery("SELECT date FROM customer");
            while (rs.next()) {
                if (rs.getString(1).equals(datetime)){
                    count++;
                }
            }
